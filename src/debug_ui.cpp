...
Bool create_debug_ui_pipeline(VulkanDevice *device, VulkanPipeline *pipeline)
{
    ...
    if (!create_pipeline(device, pipeline->render_pass, 0, &shaders, sizeof(DebugUIVertex), &vertex_attributes, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, &descriptor_sets, null,
